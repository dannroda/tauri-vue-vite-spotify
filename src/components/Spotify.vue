<script setup lang="ts">
    ...
    import { getCurrentUserQueue, getTrackInfo } from "../api/index";
    ...
    console.log('[ACCESS_TOKEN]', datos.access_token)
    console.log(datos)
    const playlists = await getCurrentUserQueue(datos.access_token);
    const currentSong = await getTrackInfo(datos.access_token, playlists.currently_playing.href)
    console.log('[CurrentSong]',currentSong)
    // <!-- <button type="button" @click="count++">count is {{ count }}</button> -->
// export default defineComponent({
// data(){
//   return {
//     name : playlists.display_name,
//   }
// },
// mounted(){
//   this.name
...
<section class="grid grid-cols-1 sm:grid-cols-3 gap-4">
      ...
      <div class="bg-gray-900 shadow-lg rounded p-3">
        <div class="group relative">
          <img class="w-full md:w-72 block rounded" :src=currentSong.album.images[0].url alt="" />
        ...
        <div class="p-5">
          <h3 class="text-white text-lg">{{currentSong.name}}</h3>
          <p class="text-gray-400">{{currentSong.artists[0].name}}</p>
        </div>
      </div>
      <!-- END OF CARD 1 -->
